...
**Dynamic:** The switch learns the first MAC address it sees on the port. However, if the switch reboots, it forgets the MAC and learns a new one when it powers back up.
...
**Shutdown (Default):** The most aggressive response. The switch instantly puts the port into an err-disable state.
