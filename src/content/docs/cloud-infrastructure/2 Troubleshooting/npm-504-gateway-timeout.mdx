...
When attempting to access the Nginx Proxy Manager (NPM) administration interface (GUI) through a domain managed by the proxy itself (e.g., `npm-admin.example.com`), the server returns an OpenResty 504 Gateway Time-out error.
