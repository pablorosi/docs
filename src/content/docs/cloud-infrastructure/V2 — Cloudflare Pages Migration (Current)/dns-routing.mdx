...
2. Add a CNAME (or use Custom domains) for the new hostname.
3. Keep backend or API routes on separate hostnames that point to the Hetzner origin.

Each static frontend gets its own Pages project and DNS record. The VPS handles only hostnames that need a running process.
